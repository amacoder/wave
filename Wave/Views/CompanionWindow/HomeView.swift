...
struct HomeView: View {
    ...
    private func deleteEntry(_ entry: TranscriptionEntry) {
        pendingUndo = entry
        withAnimation(.easeInOut(duration: 0.2)) { modelContext.delete(entry) }
    ...
    private func undoDelete() {
        ...
        withAnimation(.easeInOut(duration: 0.2)) { modelContext.insert(entry) }
        undoToastTask?.cancel()
        pendingUndo = nil
    }
}
        ...
        VStack(alignment: .leading, spacing: 16) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                ...
                Text("Welcome back, \(firstName)")
                    .font(DesignSystem.Typography.pageSubtitle)
                    .foregroundColor(.secondary)
            }

            HStack(spacing: 20) {
                StatBadge(icon: "flame.fill", iconColor: .orange, value: "\(streakDays) day\(streakDays == 1 ? "" : "s")")
                StatBadge(icon: "text.word.spacing", iconColor: DesignSystem.Colors.accentDeep, value: "\(formatNumber(totalWords)) words")
                StatBadge(icon: "gauge.open.with.lines.needle.33percent", iconColor: DesignSystem.Colors.accentDeep, value: "\(averageWPM) WPM")
            }
        }
        .padding(.horizontal, 24)
        .padding(.vertical, 20)
