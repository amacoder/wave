...
1. **Download** the latest `Wave.dmg` from [Releases](https://github.com/amacoder/wave/releases)
2. **Open** the DMG and drag **Wave.app** into your **Applications** folder
3. **Launch** Wave from Applications or Spotlight

> **First launch (unsigned build):** Wave is open-source and distributed without an Apple Developer signature, so macOS blocks the first open. Right-click **Wave.app** in Applications, choose **Open**, then **Open** again in the dialog. Alternatively run `xattr -dr com.apple.quarantine /Applications/Wave.app` once in Terminal. It launches normally afterwards.
